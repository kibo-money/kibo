...
use super::{AnyMap, Date, HeightMap, MapValue};
...
pub struct DateMap<T> {
    ...
    pub initial_last_date: Option<Date>,
    pub initial_first_unsafe_date: Option<Date>,

    imported: BTreeMap<usize, SerializedDateMap<T>>,
    to_insert: BTreeMap<usize, BTreeMap<Date, T>>,
}
    ...
    T: MapValue,
{
    ...
    pub fn insert(&mut self, date: Date, value: T) -> T {
    ...
    pub fn insert_default(&mut self, date: Date) -> T {
    ...
    pub fn get_or_import(&mut self, date: &Date) -> Option<T> {
        let year = date.year() as usize;
    ...
    #[inline(always)]
    pub fn is_date_safe(&self, date: Date) -> bool {
...
where
    T: MapValue,
{
    pub fn multi_insert<F>(&mut self, dates: &[Date], mut callback: F)
    where
        F: FnMut(&Date) -> T,
    {
    ...
    pub fn multi_insert_const(&mut self, dates: &[Date], constant: T) {
        dates.iter().for_each(|date| {
    ...
    pub fn multi_insert_complex_transform<K, F>(
        ...
        K: MapValue,
        F: FnMut((K, &Date, &mut DateMap<K>)) -> T,
    {
        dates.iter().for_each(|date| {
            self.insert(
                *date,
    ...
    pub fn multi_insert_cumulative<K>(&mut self, dates: &[Date], source: &mut DateMap<K>)
    where
        K: MapValue,
        T: LossyFrom<K>,
            ...
            let to_subtract = days
                .and_then(|x| {
                    date.checked_sub_days(Days::new(x as u64))
                        .and_then(|previous_date| source.get_or_import(&Date::wrap(previous_date)))
                })
                .unwrap_or_default();
            ...
            let previous_sum = sum.unwrap_or_else(|| {
                date.checked_sub_days(Days::new(1))
                    .and_then(|previous_sum_date| {
                        self.get_or_import(&Date::wrap(previous_sum_date))
                    })
                    .unwrap_or_default()
            });
                ...
                .unwrap_or_else(|| {
                    date.checked_sub_days(Days::new(1))
                        .and_then(|previous_average_date| {
                            self.get(&Date::wrap(previous_average_date))
                        })
                        .unwrap_or_default()
                })
                .into();
    ...
    pub fn multi_insert_net_change(&mut self, dates: &[Date], source: &mut DateMap<T>, days: usize)
    where
        T: Sub<Output = T>,
    {
        dates.iter().for_each(|date| {
            let previous_value = date
                ...
                .and_then(|date| source.get_or_import(&Date::wrap(date)))
                .unwrap_or_default();
        ...
        T: FloatCore,
    {
        ...
        let min_percentile_wdate = Date::wrap(min_percentile_date);
            ...
            if let Some(start) = days.map_or(Some(min_percentile_date), |size| {
                ...
                if sorted_vec.is_none() {
                    let mut vec = start
                        .iter_days()
                        .take_while(|d| *d <= *date)
                        .flat_map(|date| self.get_or_import(&Date::wrap(date)))
                        .filter(|f| !f.is_nan())
